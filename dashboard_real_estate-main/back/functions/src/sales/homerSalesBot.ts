...
async function incrementDailyReplies(phone: string, wasWindowExpired: boolean, collection: string): Promise<void> {
...
async function loadHistory(phone: string, collection: string): Promise<Content[]> {
  const snap = await db
