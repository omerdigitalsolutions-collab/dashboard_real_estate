...
const FOLLOW_UP_MESSAGE =
  'שלום! 😊 רק רצינו לוודא שאנחנו עדיין כאן בשבילך. האם המשכת בחיפוש הנכס? אנחנו שמחים לעזור!';
...
export const weeklyFollowUp = onSchedule(
  {
    schedule: '0 10 * * 0',
  ...
  async () => {
    ...
    for (const agencyDoc of agenciesSnap.docs) {
      ...
      for (const leadDoc of leadsSnap.docs) {
        ...
        const lastInteraction: admin.firestore.Timestamp | undefined = lead.lastInteraction;
        if (lastInteraction && lastInteraction.toDate() > sevenDaysAgo) continue;

        // Skip if already sent a follow-up within 6 days (anti-double-send)
        const lastFollowUpAt: admin.firestore.Timestamp | undefined = lead.lastFollowUpAt;
        if (lastFollowUpAt && lastFollowUpAt.toDate() > sixDaysAgo) continue;

        const phone: string | undefined = lead.phone;
        if (!phone) continue;

        try {
          const sent = await sendWhatsAppMessage(integration, phone, FOLLOW_UP_MESSAGE);
          if (sent) {
