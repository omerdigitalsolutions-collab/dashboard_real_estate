...
export const resendApiKeyForContracts = defineSecret('RESEND_API_KEY');
...
function buildClientEmailHtml(clientName: string, agencyName: string): string {
    return `
...
function buildStaffEmailHtml(clientName: string, dealId: string): string {
...
export async function notifyContractSigned(params: ContractSignedParams): Promise<void> {
    ...
    const agentId: string = dealData.createdBy || dealData.agentId || '';
    const leadId: string | undefined = dealData.leadId;
    ...
    if (!adminSnap.empty) {
        ...
        adminEmail = agencyData?.notifications?.contactEmail || adminData.email || undefined;
    } else {
    ...
    // ── 7. Send emails ────────────────────────────────────────────────────────
    const apiKey = resendApiKeyForContracts.value();
    if (!apiKey) {
