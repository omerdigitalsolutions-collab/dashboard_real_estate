...
export interface BotConfig {
  isActive: boolean;
  tone: 'professional' | 'friendly_emoji' | 'direct_sales';
  fallbackAction: 'human_handoff' | 'collect_details';
  /** Free-text guardrails from the agency's WeBot settings page */
  generalNotes?: string;
}
...
const TONE_INSTRUCTIONS: Record<BotConfig['tone'], string> = {
  professional:
    'ענה בצורה רשמית, מקצועית, אדיבה ומכובדת.',
  friendly_emoji:
    "ענה בצורה קלילה, חברית, בגובה העיניים, ושלב אימוג'ים רלוונטיים בטוב טעם.",
  direct_sales:
    'ענה בצורה קצרה, עניינית, מוכוונת מטרה, ודחוף בעדינות לקביעת פגישה או סיור בנכס.',
};

const FALLBACK_INSTRUCTIONS: Record<BotConfig['fallbackAction'], string> = {
  human_handoff:
    'התנצל בנימוס והסבר שסוכן אנושי מהמשרד יחזור אליו בהקדם האפשרי.',
  collect_details:
    'בקש מהלקוח לפרט קצת יותר: איזה אזור הוא מחפש, כמה חדרים, ומה התקציב, כדי שנוכל לעזור לו טוב יותר.',
};
...
export function buildWeBotPrompt(config: BotConfig, properties: Property[]): string {
  ...
  const toneText =
    TONE_INSTRUCTIONS[config.tone] ?? TONE_INSTRUCTIONS['professional'];

  const fallbackText =
    FALLBACK_INSTRUCTIONS[config.fallbackAction] ??
    FALLBACK_INSTRUCTIONS['human_handoff'];
  ...
  const agencyNotes =
    config.generalNotes && config.generalNotes.trim() !== ''
