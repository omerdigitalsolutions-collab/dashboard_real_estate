...
exports.linkEventToEntity = linkEventToEntity;
exports.unlinkEventFromEntity = unlinkEventFromEntity;
 ...
 * ```
 */
async function linkEventToEntity(entityType, entityId, eventId) {
    ...
    await entityRef.update({
        ...
        calendarLinkedAt: firestore_1.FieldValue.serverTimestamp(),
    });
    ...
    console.info(`[calendar] Linked event ${eventId} → ${entityType} ${entityId} (collection: ${collection})`);
}
 ...
 */
async function unlinkEventFromEntity(entityType, entityId, eventId) {
    const collection = COLLECTION_MAP[entityType];
    const entityRef = db.collection(collection).doc(entityId);
    await entityRef.update({
        calendarEventId: firestore_1.FieldValue.delete(),
        calendarLinkedAt: firestore_1.FieldValue.delete(),
    });
    const linkRef = db.collection(CALENDAR_LINKS_COLLECTION).doc(eventId);
    await linkRef.delete();
    console.info(`[calendar] Unlinked event ${eventId} from ${entityType} ${entityId}`);
}
