...
export default function AiExpenseImporter({ onImported }: { onImported?: () => void }) {
    ...
    const [aiResults, setAiResults] = useState<ParsedExpense[]>([]);
        ...
        if (fileName.endsWith('.csv')) {
            ...
            try {
                ...
                if (workbook.SheetNames.length === 0) {
                    throw new Error('קובץ האקסל ריק');
                }
                const firstSheetName = workbook.SheetNames[0];
                const worksheet = workbook.Sheets[firstSheetName];
                const rawJson = XLSX.utils.sheet_to_json(worksheet, { defval: '' }); // keep empty cells as ''
                if (rawJson.length === 0) {
                    throw new Error('לא נמצאו נתונים בגיליון הראשון');
                }
    ...
    // Step 2: Send raw JSON to Gemini
    const callGemini = async (rawJsonArray: any[]) => {
        try {
            setStatus('ai_processing');
            // Stringify and slice to avoid giant payloads if file is huge
            const payloadString = JSON.stringify(rawJsonArray).slice(0, 30000); // Rough limit 

            const extractFn = httpsCallable<any, { success: boolean, data: ParsedExpense[] }>(functions, 'ai-extractAiData');
    ...
    // Step 3: Toggle recurring in memory before saving
    ...
    const handleSave = async () => {
        try {
            setStatus('saving');
            // Instead of batch, we can just Promise.all with addExpense
            const promises = aiResults.map(item => {
    ...
    return (
        <div className="bg-slate-900/60 backdrop-blur-xl border border-blue-500/20 rounded-2xl p-6 shadow-[0_0_30px_rgba(59,130,246,0.05)] text-right" dir="rtl">
            ...
            <p className="text-sm text-slate-400 mb-6 font-medium">
                העלה קובץ באקסל / CSV (למשל מפירוט אשראי). ה-AI שלנו יקרא את ההוצאות ויקטלג אותן אוטומטית.
            </p>
            ...
            {(status === 'parsing' || status === 'ai_processing' || status === 'saving') && (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <p className="font-bold text-lg text-white">
                        ...
                        {status === 'ai_processing' && 'Gemini AI מנתח ומקטלג את ההוצאות שלך...'}
                        {status === 'saving' && 'שומר למערכת...'}
                    </p>
            ...
            {status === 'review' && aiResults.length > 0 && (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="flex items-center justify-between mb-4 mt-2">
                        <h3 className="font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-3 py-1.5 rounded-lg flex items-center gap-2">
                            <CheckCircle2 className="w-4 h-4" />
                            {aiResults.length} הוצאות חולצו בהצלחה
                        </h3>
                        <button
                            onClick={handleSave}
                        ...
                        <table className="w-full text-sm text-right">
                            <thead className="bg-slate-800 text-slate-400">
                                <tr>
                                    ...
                                    <th className="p-3 font-semibold">קטגוריה (AI)</th>
                                    <th className="p-3 font-semibold">תאריך</th>
                                    <th className="p-3 font-semibold text-center">הוצאה קבועה?</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-800">
                                {aiResults.map((item, idx) => (
                                    <tr key={idx} className="hover:bg-slate-800/40 transition-colors">
                                        ...
                                        <td className="p-3 font-bold text-rose-400" dir="ltr">₪{item.amount.toLocaleString()}</td>
                    ...
                    <p className="text-xs text-slate-500 mt-3 text-center">
                        אנא עבר על הטבלה וודא שהבינה המלאכותית צדקה בקטלוג, וסמן "הוצאה קבועה" (למשל שכירות או מנוי חודשי) במקומות הנדרשים כדי שזה יחושב כל חודש אוטומטית בדוח.
                    </p>
                </div>
            )}
        </div>
